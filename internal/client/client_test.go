...
func TestClientGetConversationHistory(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			messages, err := client.GetConversationHistory(context.Background(), tc.limit, nil)
			if tc.expectError && err == nil {
				t.Errorf("Expected error but got nil")
			} else if !tc.expectError && err != nil {
				t.Errorf("Expected no error but got: %v", err)
...
func TestClientSearchMessages(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			messages, err := client.SearchMessages(context.Background(), tc.query, tc.limit)
			if tc.expectError && err == nil {
				t.Errorf("Expected error but got nil")
			} else if !tc.expectError && err != nil {
				t.Errorf("Expected no error but got: %v", err)
			...
			if !tc.expectError && len(messages) != len(tc.expectedMessages) {
				t.Errorf("Expected %d messages but got %d", len(tc.expectedMessages), len(messages))
			}
		})
	}
}
