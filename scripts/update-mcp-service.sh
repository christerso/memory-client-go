#!/bin/bash
# Script to update the Memory Client MCP service on Linux with the new tagging features
...
OLD_SERVICE_NAME="memory-client-mcp"
SERVICE_NAME="memory-client-mcp-service"
...
EOF
    ...
    echo "Updating existing configuration with tagging settings..."
    # Check if tagging section exists, if not add it
    if ! grep -q "tagging" "$CONFIG_FILE"; then
        # Use temporary file to avoid issues with inline editing
        TMP_FILE=$(mktemp)
        jq '. + {"tagging": {"defaultMode": "automatic", "bufferSize": 5, "categories": ["technical", "planning", "question", "feedback"]}}' "$CONFIG_FILE" > "$TMP_FILE"
...
if systemctl is-active --quiet "$SERVICE_NAME"; then
    echo "Memory Client MCP service has been updated and started successfully."
else
...
echo ""
echo "Memory Client MCP Service has been updated with the new tagging features."
echo ""
